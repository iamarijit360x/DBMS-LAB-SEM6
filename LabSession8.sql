...
CREATE VIEW NUMOFSALP AS SSELECT CITY,COUNT(SNUM) FROM SALESPEOPLEX GROUP BY CITY;
CREATE VIEW TOTALORD SELECT SNUM,COUNT(ONUM) AS TOTALORDERS FROM ORDERSX GROUP BY SNUM;
CREATE VIEW AVGORD SELECT AVG(COUNT(ONUM)) FROM ORDERSX GROUP BY SNUM;
