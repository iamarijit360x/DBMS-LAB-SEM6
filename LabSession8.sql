...
SELECT CNAME FROM CUSTOMERX WHERE RATTING=(SELECT MAX(RATTING) FROM CUSTOMERX);
SELECT CITY,COUNT(SNUM) FROM SALESPEOPLEX GROUP BY CITY;
SELECT SNUM,COUNT(ONUM) AS TOTALORDERS FROM ORDERSX GROUP BY SNUM;
SELECT AVG(COUNT(ONUM)) FROM ORDERSX GROUP BY SNUM;
SELECT SNAME FROM SALESPEOPLEX WHERE SNUM IN (SELECT SNUM FROM CUSTOMERX GROUP BY SNUM HAVING COUNT(CNUM)>1);
...
WHERE COMM BETWEEN 1000 AND 2000 WITH CASCADE CHECK OPTION;